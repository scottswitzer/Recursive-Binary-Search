...
// on the DE1-SoC to interact with binary_search. When 
// started, the code enters a loop that continuously 
// checks the values on SW0-SW9 and displays them on 
// LEDR0-LEDR9 until you press KEY3. Then, it initializes 
// numbers and calls binary_search. After binary_search 
// returns, it will display the result on LEDR0-LEDR9 and 
// waits for you to press KEY0. After pressing KEY0 the 
...
wait_key3:
      ...
      // NOTE: str r7,[r6] is at address 0x00000010. If  
      // binary_search "jumps" to the above instruction this 
      // is likely because an LDR or STR instruction inside
      // binary_search accesses an address that is NOT a 
      // multiple of 4. Using LDR or STR with such an address 
      // triggers a ``Data Abort'' exception, which in turn
      // causes the Cortex-A9 to set the PC to 0x00000010.  
      // Debug tip: Check you write -numCalls to the right 
      // address in memory.
      ...
      mov r3,#99          // R3 = endIndex
      mov r8,#0           // initial NumCalls
      str r8,[sp,#-4]     // save 5th arg to stack
      bl  binary_search   // call binary_search    
      str r0,[r6]         // display result on LEDR0-LEDR9
